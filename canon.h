...
class canon {
 ...
 -4, 0.5,0, // vertex 4
      ...
      wheel = create3DTexturedObject(GL_TRIANGLES, 6, vertex_buffer_data2, texture_buffer_data, textureID[1], GL_FILL);
    ...
    void plotPath() {
      ...
      int ct=0;
      while(x <= zoom_scale && y<=zoom_scale && x>=-zoom_scale && y>=-zoom_scale &&x<=-2) {
        x += vx*0.01 - 0.5*drag*0.0001;
        y += vy*0.01 - 0.5*gravity*0.0001;
        vy -= gravity*0.01;
        vx -= drag*0.01;
        if(vx<=0) vx = 0; 
        // if(vy<=0) vy = 0; 
